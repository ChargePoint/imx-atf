...
static void wdog1_cpu(uint32_t addr) {
  ...
  mmio_write_32((uintptr_t)&wdog->cs,
                (0x2900 | WDGCS_CMD32EN | WDGCS_WDGE | WDGCS_WDGUPDATE |
                 (WDG_LPO_CLK << 8) | WDGCS_FLG));
...
int imx_src_handler(uint32_t smc_fid, u_register_t x1, u_register_t x2,
  ...
  case IMX_SIP_SRC_M4_START:
    ...
    if (x2)
      ...
      mmio_write_32(REG_SRC_GLOBAL_GPR9,
                    0xFFE0000); /* for SPL kick M33, Uboot load m33 firmware */
